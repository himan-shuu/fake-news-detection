...
def load_assets():
    ...
    try:
        ...
        # CRITICAL FIX: Get the actual input names the model is expecting (e.g., ['input_1', 'input_2'])
...
def preprocess_text(text, tokenizer, max_len):
    ...
    # Return the encoded tensor parts
    return encoded['input_ids'], encoded['attention_mask']


# --- Prediction Function ---
# CRITICAL CHANGE: Added input_names argument
def predict_fake_news(text, tokenizer, model, input_names):
    ...
    # CRITICAL FIX: Create the input dictionary using the actual layer names
    inputs_dict = {
        input_names[0]: input_ids,
        input_names[1]: attention_mask
    }
    
    # Predict - This dictionary format guarantees input compatibility
...
def main():
    ...
    # --- Load Assets ---
    # CRITICAL CHANGE: Unpack the new input_names variable
    ...
    if st.button("Classify Article", use_container_width=True, type="primary"):
        with st.spinner('Analyzing content...'):
            # CRITICAL CHANGE: Pass input_names to the prediction function
